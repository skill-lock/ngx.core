 ...
 * @template TStateItem state item typing.
 * @param {(string | number)} key key to find and update.
 * @param {TState} state original state reference.
 * @param {PartialObject<TStateItem>} changes item changes to be added/updated.
 ...
 *
 * @template TState state typing.
 * @template TStateItem state item typing.
 * @param {TState} state original state reference.
 * @param {PartialObject<TStateItem>[]} changes list of changes to be added/updated.
 * @param {string} [keySelector="id"] Property name used as the key within the model.
...
export function updateAllMapState<TState extends Dictionary<TStateItem>, TStateItem>
	(state: TState, changes: PartialObject<TStateItem>[], keySelector = "id"): TState {
